...
import shopbg4 from '../Shop/Assets/shopbg4.png';
...
const Shop = ({prodId, title, price, image, imageTwo, onAddToCart}) => {
    ...
    return (
        <ShopHome>
            <div>
                ...
                <ShopRows>
                    {productData.map((prods, prodId) => {
                        return (
                            <ProductMain key={prods.prodId} id={prods.prodId} title={prods.prodId} onAddToCart={onAddToCart}>
                                <ProductInfo>
                                    <p>{prods.title}</p>
                                    <ProductPrice>
                                    <small>$</small>
                                ...
                                <div style={{display:'flex'}}> 
                                    <ProdButton onClick={() => singlePorductRedirect(prodId)}>View More</ProdButton>
                                    <ProdButton onClick={() => onAddToCart(prods.prodId, 1/*product.id*/ )}>Add to Cart</ProdButton>
                                </div>
                            </ProductMain>
                        )
                    })}
